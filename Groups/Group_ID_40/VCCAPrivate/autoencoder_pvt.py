...
ZDIMS=20
PDIMS=30
class AutoencoderPrivate(nn.Module):
    def __init__(self,Zdims,Pdims,input_dim):                                                  
        super(AutoencoderPrivate, self).__init__()
        self.private = True
        # ENCODER
        ...
        self.en_x_1 = nn.Linear(input_dim, 1024)
        ...
        self.en_y_1 = nn.Linear(input_dim, 1024)
    ...
    def transformPrivate(self,x_view,view):
        mu, log_var = self.encode(x.view(-1, self.input_dim))

        mu1, log_var1 = self.private_encoder1(x.view(-1, self.input_dim))
        mu2, log_var2 = self.private_encoder2(y.view(-1, self.input_dim))
