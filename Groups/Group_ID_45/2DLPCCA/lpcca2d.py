...
class LPCCA2D:
    ...
    def fit(self):
        ...
        A1=np.dot(np.dot(np.dot(lpcca2d_C_xy,cov_yy_inv),lpcca2d_C_yx),cov_xx_inv)
        
        A2=np.dot(np.dot(np.dot(lpcca2d_C_yx,cov_xx_inv),lpcca2d_C_xy),cov_yy_inv)
...
class LPCCA2DHeatKernel:
    ...
    def meansquaredistance(self,X):
        return squareform(pdist(X,'sqeuclidean'))
        
    def similarity_matrix(self,X,k):
        mean_squared_distances=self.meansquaredistance(X)
        NintoNminus1=(mean_squared_distances.shape[0])*(mean_squared_distances.shape[0]-1)
        tx=mean_squared_distances.sum()*2/NintoNminus1
        k_nearest_neighbours = NearestNeighbors(n_neighbors=k).fit(X)
        k_distances, k_indices = k_nearest_neighbours.kneighbors(X) 
        similarity=np.zeros([X.shape[0],X.shape[0]])
        for i in range(X.shape[0]):
            for j in range(k):
                similarity[i][k_indices[i][j]]=math.exp(-mean_squared_distances[i][k_indices[i][j]]/tx)
        return similarity
