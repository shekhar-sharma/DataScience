...
class LapMCCA:
  ...
  # function to create affinity graph
  ...
  # function to create within-view covariance using D,W,L
  ...
  # function to create between-view covariance using D,W,L
  ...
  def fit(self,X,label):
    ...
    for i in range(m):
      ...
      for j in range(m):
        if(i!=j):
          Si.append(self.S_i_j(X[i],Lij[i][j],X[j].T,n))
        else:
          Si.append(0)
      Sij.append(Si)
  ...
  # method to fit and transform the model
